...
def preencher_tv(tree):
    ...
    for produto_estoque, cd_produto, nm_produto, cd_estoque, nr_rua, nr_prateleira, nr_sequencia in produtos_estoque:
        tree.insert("", "end", values=(cd_produto,nm_produto, cd_estoque, nr_rua, nr_prateleira, nr_sequencia, produto_estoque.nr_lote, produto_estoque.qt_produtoestoque, produto_estoque.dt_validade, produto_estoque.dt_produtoestoque))
...
def lista_produtos_estoque():
    ...
    tview = ttk.Treeview(lista_produtos_estoque, columns=("ID","Produto","ID-Estoque", "Rua", "Prateleira", "Sequencia","Lote", "Quantidade","Vencimento", "Dt Registro"), show='headings')
    ...
    tview.column("#3", width=0, stretch=NO)
